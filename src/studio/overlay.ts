...
(function(){
        ...
        function pump(){ return reader.read().then(function(res){
          ...
          for(var i=0;i<blocks.length;i++){
            ...
            if(msg.kind==='status'){ setStatus(msg.text); }
              ...
              if(msg.error){ setStatus(msg.error,true); gen.disabled=false; if(msg.auth==='expired'){ loadLog(); } }
  ...
  function reset(){
    ...
    setStatus('Resetting...');
    fetch('/api/remix/reset',{method:'POST'}).then(function(){ try{localStorage.removeItem(COOKIE);localStorage.removeItem(FREE_KEY);}catch(e){} clearCookie(); location.reload(); })
      .catch(function(e){ setStatus(String(e),true); });
  }
})();
</script>`;
